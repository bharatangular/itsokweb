...
export class NewheaderComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.empinfo=this.tokenInfo.empinfoService;
     ...
     let rolelist:any=sessionStorage.getItem("selected-roledetail");
     if(rolelist)
     {
      this.decLevelData= this.config.decrypt(rolelist);
      this.decLevelData = JSON.parse(this.decLevelData);
     }
    
    this.empShowDetails.Office_ID=this.empinfo?.levelValueCode && this.empinfo?.levelValueCode;
    this.empShowDetails.offcice_details=this.empinfo?.levelValueCode && this.empinfo?.levelValueCode;
    this.empShowDetails.roleName=this.empinfo?.roleName && this.empinfo?.roleName;
   
    this.ssoimg = sessionStorage.getItem("ssoimg");
    if (this.ssoimg==undefined || this.ssoimg==null || this.ssoimg=='') {
...
//     )
  ...
  redirectToDashboard(j: any) {
   // alert("Hi");
    ...
    if(this.roleList[j].payeeRoleId=='99')
    {
      this.router.navigate(['pension/dashboard']);
    }else
    {
