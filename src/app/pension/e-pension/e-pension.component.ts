...
@Component({
  selector: 'app-e-pension',
  templateUrl: './e-pension.component.html',
  styleUrls: ['./e-pension.component.scss'],
  providers: [
...
export class EPensionComponent implements OnInit {
  ...
  constructor(
    ...
    private redirectMultiService: RedirectMultiService
  ) {
  ...
  ngOnInit(): void {
    ...
    this._Service.configMenu = { url: "Inbox  >  Profile" };
...
}else if(this.psnTypeId=="69")
{
  ...
  uploadFileDirect(event: any, i: any) {
    ...
    reader.onloadend = () => {
      ...
      this._Service.postOr("wcc/uploaddocs", data1).subscribe({
        next: (res: any) => {
          console.log("res", res);
          if (i == 1)
            this.isLoadingSPic = false
      ...
      this._Service.postRequestpension(data, 'callPensionCalculationRuleEngine').subscribe({
        next: (res) => {
          ...
          if ((res.status = 200)) {
            ...
            if (Number(this.Pension_Commutation?.netPay) < 1) {
  ...
  getAllUploadedDocumentDetailsByEmployeeCode() {
    ...
    this._Service.postRequestpension(data, 'getDocsDtlsBySubId').subscribe((res) => {
      if (res.status = 200) {
        ...
        console.log("documentlist 1", this.documentlist);
        let data = {
          "dmsDocId": null,
          "docName": "FR",
          "docTypeId": 36
  ...
  onSubmit_Final_Maker(action: any) {
    ...
    if (this.serviceRecordDatanew.deStatus == '') {
    ...
    this.ServiceDetails['psnTypeId'] = this.psnTypeId ? this.psnTypeId : "";
    ...
    let docCheckj: any[] = []
    docCheck = this.documentlist.filter((x: any) => x.docTypeId == 33)
    docCheckj = this.documentlist.filter((x: any) => x.docTypeId == 32)
    ...
    {
      this.documentlist.map(item=>{if(item.docTypeId==33){
        return {...item,dmsDocId:this.Personaldetail?.selfPhotoId}
      }
      return item;
    });
     
    }
    if (docCheckj.length == 0) {
      let jointPic = {
    ...
    {
      this.documentlist.map(item=>{if(item.docTypeId==32){
        return {...item,dmsDocId:this.Personaldetail?.jointPhotoId}
      }
      return item;
    });
  ...
  //       "condition_Amount2": "",
  //       "condition_Amount": this.conditionForm.value.condition_1_Amount,
  //       "condition_string": "It is certified that" + (!this.isCrimnalCase ? ' No ' : "") + "Departmental Enquiry under rule 16 of C.C.A Rules or criminal case is pending or contemplated against the employee on dated " + this.date.transform(this.conditionForm.value.condition_1_Date, "dd-MM-yyy") + "(date of submission of pension case) as per appointing authority order No " + this.conditionForm.value.condition_1_Amount + " date " + this.date.transform(this.conditionForm.value.condition_1_Date1, "dd-MM-yyy") + " . Copy of retirement orders and no D.E. certificate issued from the Appointing Authority are attached with e-pension set"
  //     },
  //     {
  //       "condition_Amount2": "",
  //       "condtion_type_id": "2",
  ...
  getBudgetDetails(ser: any) {
    ...
    let data =
    {
      "pensionCode": this.psnTypeId ? this.psnTypeId : "68",
    ...
    this._Service.postdeduction(data, 'getPensionBudgetHead').subscribe({
      next: (res) => {
        ...
        this.pType=this.BudgetDetails[0].pensionType
    ...
    this._Service.requestApplication(data, "task").subscribe({
      next: (res) => {
        if (res.status = 200) {
          ...
          // if(Number(this.psnTypeId)==68)
          // {
          // this.pType="Superannuation"
          // }else if(Number(this.psnTypeId)==69)
          // {
          //   this.pType="Voluntary Retirement Pension U/R 50"
          // }
  ...
  removeAPC(i: any) {
    ...
    this.docList = this.docList.filter((x: any) => x.docName != 'Accounts Personnel Certificate');
    this.documentlist = this.documentlist.filter((x: any) => x.docName != 'Accounts Personnel Certificate');
  ...
  multiesign() {
    this.config.storeDetails("multiesign","1")
    let time1 = new Date();
    let mainUrl = window.location.origin + "pension/#" + "/pension/e-Pension/Profile";
