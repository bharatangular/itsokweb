...
@Component({
  selector: 'app-request-mark-vrs',
  templateUrl: './request-mark-vrs.component.html',
  styleUrls: ['./request-mark-vrs.component.scss']
})
export class RequestMarkVrsComponent implements OnInit {

  
  displayedColumns: string[] = ['employeeCode','name','VRS Date','DOR','requestDate',"document","remark","Approve","revert"];
  displayedColumns1: string[] = ['employeeCode','name','VRS Date','DOR','requestDate',"document",];
...
config:AppConfig=new AppConfig()
  ...
  ngOnInit(): void {
    this._Service.configMenu = { url: "Request VRS List" };
...
this.load.show();
this._Service.postCumm("requestVRS", data).subscribe({
...
approveLifeCertificate(row:any,flag:any,i:any)
{
  ...
  if(confirm("Are you sure..."))
  {
    ...
    if(flag==1)
    {
      data={
        'inType':4,
        'assignmentId':Number(this.userdetails.assignmentid),       
        'hoRemark': this.remark[i],
