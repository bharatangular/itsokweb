...
@Component({
  selector: 'app-total-retirement-category',
  templateUrl: './total-retirement-category.component.html',
  styleUrls: ['./total-retirement-category.component.scss']
})
export class TotalRetirementCategoryComponent implements OnInit {
...
//   }
  ...
  applyFilter(event: Event) {
    ...
    generateExcelOrPdfData() {
      ...
      const dataHeaderArray = [
       {
        sNo:"S No.",
        serviceCate: "Service Category", 
        totalServcRetiree: "Total Service Retirees",
        ifms2: "IFPMS 2.0", 
  ...
  for(const eachData of filteredData) {
    let eachObj: any = {}
      ...
      eachObj["serviceCate"] = eachData?.serviceCate ?? "" ;
      eachObj["totalServcRetiree"] = eachData?.totalServcRetiree ?? "";
  ...
  exportToExcel(name: string){
    ...
    this.loader.show()
    let  [dataHeaderArray, firstPensionList] = this.generateExcelOrPdfData()
    this.commonService.exportTOExcel(firstPensionList, dataHeaderArray,"Service Category wise Retirement.")
    this.loader.hide()  
  }
  }
