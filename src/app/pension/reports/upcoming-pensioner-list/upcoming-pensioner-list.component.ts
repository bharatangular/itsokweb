...
@Component({
  selector: 'app-upcoming-pensioner-list',
  templateUrl: './upcoming-pensioner-list.component.html',
  styleUrls: ['./upcoming-pensioner-list.component.scss']
})
export class UpcomingPensionerListComponent implements OnInit,AfterViewInit  {

  @Output() EmpId = new EventEmitter();

  displayedColumns: string[] = ['employeeCode','name','deptName','designationName','dateOfBirth','dateOfJoining','dateOfRetirement','essInitated',"psnInitated"];
...
//   }
  let data = {
    ...
    'isDate': this.isDate
    // 'inRole':this.userdetails.roleid
  }

  this.load.show();
  this._Service.postCumm( "getemployeecommonreportbyreporttype",data).subscribe({
      next: (res:any) => {
  ...
  exportToPdf(){
    ...
    this.load.show()
    this.commonService.downloadPdf("#upcoming_Pensioner","upcoming_Pensioner", "Upcoming pensioner")
    this.load.hide()
  }
  ...
  exportToExcel(){
    ...
    this.load.show()
    this.commonService.ExportTOExcel(pensionerList,"Upcoming pensioner")
    this.load.hide()
