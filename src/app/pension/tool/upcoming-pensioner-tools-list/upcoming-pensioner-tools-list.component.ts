...
@Component({
  selector: 'app-upcoming-pensioner-tools-list',
  templateUrl: './upcoming-pensioner-tools-list.component.html',
  styleUrls: ['./upcoming-pensioner-tools-list.component.scss']
})
export class UpcomingPensionerToolsListComponent implements OnInit {
...
empCode: any;
  ...
  constructor(
    ...
    // @Inject(MAT_DIALOG_DATA) public matdata: {message: any},
    // private dialogRef: MatDialogRef<ViewPensionEssComponent>
  ) {
...
this.ObjectionArray[i].rejectedDate=this._Service.currentDate;
  ...
  fetchPersonaldetail() {
    ...
    console.log("fetching Personal Details : " ,data)
...
console.log(this.ObjectionArray)
  ...
  getEMPID(){
    console.log("empId here : ");

    let data={
      'inType':1,
      'employeeId':this.empCode
    }
    console.log("data",data)
    this._Service.postNewEmployee("getEmployeeDetailsByType",data).subscribe((res:any)=>{
      console.log("res",res)
      if(res.data)
      {
        console.log("res2",res.data.employeePersonalDetail.employeeId)
        this.employeeId=res.data.employeePersonalDetail.employeeId;
        console.log ("empId here : " + this.employeeId)
...
//console.log(this.treasurylist);
  ...
  onSubmit_Final_Maker(action: any) {
    ...
    let condition_data = [{
      ...
      "condition_string": "It is certified that" + (!this.isCrimnalCase ? ' No ' : "") + "Departmental Enquiry under rule 16 of C.C.A Rules or criminal case is pending or contemplated against the employee on dated " + this.date.transform(this.conditionForm.value.condition_1_Date, "dd-MM-yyy") + " as per appointing authority order No " + this.conditionForm.value.condition_1_Amount + " date " + this.date.transform(this.conditionForm.value.condition_1_Date, "dd-MM-yyy") + " (date of submission of pension case). Copy of retirement orders and no D.E. certificate issued from the Appointing Authority are attached with e-pension set"
    },
    {
      "condition_Amount2": "",
      "condtion_type_id": "2",
          ...
          this.dialog.open(CommonDialogComponent,
            {
              panelClass: 'dialog-w-50', autoFocus: false
              , data: {
                message: 'success', reqId: this.rid, id: 4, aid: this.empinfo.aid, ppo_no: ''
              }
            }
          );
        }
